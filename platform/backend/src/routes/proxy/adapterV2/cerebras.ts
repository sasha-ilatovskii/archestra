...
type CerebrasRequest = Cerebras.Types.ChatCompletionsRequest;
type CerebrasResponse = Cerebras.Types.ChatCompletionsResponse;
...
class CerebrasRequestAdapter
  implements LLMRequestAdapter<CerebrasRequest, CerebrasMessages>
{
  readonly provider = "cerebras" as const;
  private request: CerebrasRequest;
  private modifiedModel: string | null = null;
  private toolResultUpdates: Record<string, string> = {};

  constructor(request: CerebrasRequest) {
  ...
  getProviderMessages(): CerebrasMessages {
    return this.request.messages;
  }

  getOriginalRequest(): CerebrasRequest {
    return this.request;
  }
  ...
  toProviderRequest(): CerebrasRequest {
  ...
  private toCommonFormat(messages: CerebrasMessages): CommonMessage[] {
    logger.debug(
      { messageCount: messages.length },
      "[CerebrasAdapter] toCommonFormat: starting conversion",
    );
  ...
  private applyUpdates(
    ...
    if (updateCount === 0) {
      logger.debug("[CerebrasAdapter] applyUpdates: no updates to apply");
      return messages;
    }
    ...
    const result = messages.map((message) => {
      if (message.role === "tool" && updates[message.tool_call_id]) {
        ...
        logger.debug(
          { toolCallId: message.tool_call_id },
          "[CerebrasAdapter] applyUpdates: applying update to tool message",
        );
        return {
          ...message,
...
class CerebrasResponseAdapter implements LLMResponseAdapter<CerebrasResponse> {
  readonly provider = "cerebras" as const;
  private response: CerebrasResponse;

  constructor(response: CerebrasResponse) {
...
class CerebrasStreamAdapter
  implements LLMStreamAdapter<CerebrasStreamChunk, CerebrasResponse>
{
  readonly provider = "cerebras" as const;
  ...
  processChunk(chunk: CerebrasStreamChunk): ChunkProcessingResult {
  ...
  formatEndSSE(): string {
    const finalChunk: CerebrasStreamChunk = {
      id: this.state.responseId,
      object: "chat.completion.chunk",
      created: Math.floor(Date.now() / 1000),
      model: this.state.model,
  ...
  toProviderResponse(): CerebrasResponse {
    const toolCalls =
      ...
      choices: [
        {
          ...
          logprobs: null,
          finish_reason:
            (this.state.stopReason as Cerebras.Types.FinishReason) ?? "stop",
        },
      ],
      usage: {
...
async function convertToolResultsToToon(
  ...
  // Use tiktoken tokenizer (same as OpenAI) for Cerebras
...
export const cerebrasAdapterFactory: LLMProvider<
  CerebrasRequest,
  CerebrasResponse,
  CerebrasMessages,
  CerebrasStreamChunk,
...
> = {
  ...
  ): LLMRequestAdapter<CerebrasRequest, CerebrasMessages> {
    return new CerebrasRequestAdapter(request);
  },

  createResponseAdapter(
    response: CerebrasResponse,
  ): LLMResponseAdapter<CerebrasResponse> {
  ...
  extractApiKey(headers: CerebrasHeaders): string | undefined {
    ...
    return {
      [Symbol.asyncIterator]: async function* () {
        for await (const chunk of stream) {
          yield chunk as CerebrasStreamChunk;
        }
      },
    };
  },
